...
def main():
    ...
    forward_button['command'] = lambda: func(left_speed_entry, right_speed_entry)
    ...
    root.mainloop()
    #next(left_speed_entry, right_speed_entry)

def func(left, right):

    mqtt_client = com.MqttClient()
    mqtt_client.connect('lara', 'nathalie')
    time.sleep(1)  # Time to allow the MQTT setup.
    print()

    mqtt_client.send_message("forward", [left.get(), right.get()])
